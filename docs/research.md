...
This suite therefore treats Code Maat as the preferred optional language-agnostic behavioral hotspot provider, preserves its metric definitions/ranges, and supplies a small adapter rather than coupling consuming skills to raw CSV. Its native revisions and entity-churn outputs can be combined for hotspot ranking. The default Git provider offers a zero-install fallback of change frequency plus tracked physical LOC; no ecosystem-specific complexity analyzer is required. Code Maat is not mandatory because it requires a JVM/container, works from prepared logs, can use substantial memory, and may not already be present in a private-repository environment.

Behavioral concepts adopted from Adam Tornhill's published work and Code Maat documentation include hotspot candidate ranking, change frequency plus size/churn, temporal/logical coupling, ownership concentration, changeset-size filtering, and component mappings. The fallback ranks change frequency and LOC separately before multiplying the ranks, retaining the two-dimensional hotspot intent without inventing a universal debt score. The suite refuses the stronger unsupported leap from a metric to technical debt or bad architecture.
