...
Git is the zero-install history provider. Code Maat support is an adapter over selected CSV analyses, not a dependency. Ecosystem tools extend capabilities through the same observation envelope when their definitions and units are preserved.
...
The repository identifier hashes the origin URL when present or the root commit otherwise, so moving a clone does not inherently invalidate history evidence. HEAD, provider and analyzer versions, parameters, schema, and analysis inputs do. Dirty state is recorded, but only invalidates evidence that consumed working-tree content. Code Maat adapter provenance also hashes its raw CSV input.
